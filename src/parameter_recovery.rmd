...
binary_update <- function(col){ifelse(col == 1, 0.999, 0.001)}
...
mod <- cmdstan_model(file, cpp_options = list(stan_threads = TRUE), stanc_options = list("O1"))
...
ggplot(data = weights_draws_df) +
    geom_density(
      aes(x=weights, fill=as.character(c_parameter),
      color=as.character(c_parameter)), alpha=.2
      ) +
    geom_vline(aes(xintercept=w_parameter), linetype = "dashed") +
    theme_bw() + facet_wrap(.~feature_number)
```
