...
```{r}
# save df
# write.csv(recovery_df, paste("../data/recovery/parameter_recovery_gcm", type, n_obs, ".csv", sep = ""))
```
...
# model parameters
# alpha_parameters <- seq(from = 0, to = 1, length.out = 6)
alpha_parameters <- c(0, 0.2, 0.4, 0.6)
temp_parameters <- seq(from = 0, to = 10, length.out = 6)

# run model
parameters <- tibble(expand_grid(n_obs = n_obs,
                                 n_features = n_features,
                                 type = "binary",  # only works with binary features
                                 alpha = alpha_parameters,
...
for (true_alpha in alpha_parameters){
  for (true_temp in temp_parameters){
    draws_df <- param_recov_rl(n_obs = 300, n_features = 5, type = "binary", alpha = true_alpha, temp = true_temp)
    
    if (exists("test_df")) {
        test_df <- rbind(test_df, draws_df)
    } else {
        test_df <- draws_df
    }
  }
...
ggplot(data = recovery_df) +
    ...
    labs(title = "Prior Posterior update: Alpha_negative parameter",
         caption = "Prior (blue) and posterior (red)") + 
    xlab("Alpha_negative")
```
...
ggplot(data = recovery_df) +
    ...
    theme_bw() +
    facet_wrap(. ~ true_alpha) + 
    labs(title = "Prior Posterior update: Alpha_positive parameter",
         caption = "Prior (blue) and posterior (red)") + 
    xlab("Alpha_positive")
```
...
ggplot(data = recovery_df) +
    ...
    theme_bw() +
    facet_wrap(. ~ true_alpha) + 
    labs(title = "Posterior update: Alpha_negative parameter",
...
ggplot(data = recovery_df) +
    ...
    theme_bw() +
    facet_wrap(. ~ true_alpha) + 
    labs(title = "Posterior update: Alpha_positive parameter",
...
ggplot(data = recovery_df) +
    ...
    labs(title = "Prior Posterior update: Temperature parameter",
         caption = "Prior (blue) and posterior (red)") + 
    xlab("Temperature")
```
```{r}
ggplot(data = recovery_df) +
    ...
    theme_bw() +
    facet_wrap(. ~ true_temp) + 
    labs(title = "Posterior update: Temperature parameter",
         fill = "Alpha parameter") + 
    xlab("Temperature")
```