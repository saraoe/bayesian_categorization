...
ggsave("figs/rl/model_validation/responses_cumulative.png",
    width = 7, height = 10
)
```
...
ggsave("figs/rl_simple/model_validation/prior-pos-update-alpha_neg.png",
    width = 7, height = 5
)
```
...
```{r}
ggplot(data = response_df) +
    geom_point(aes(x = trial, y = cumulative, color = alpha),
        alpha = .3
    ) +
    geom_smooth(aes(x = trial, y = cumulative, color = alpha)) +
    theme_bw() +
    facet_wrap(. ~ temp_round) +
    labs(
        color = latex2exp::TeX("$\\alpha$")
    ) +
    scale_color_brewer(palette = "PuOr")
